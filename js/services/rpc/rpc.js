...
function(syscall, time, alerts, utils, rootScope, uri, authconf) {
  ...
  var update = function() {
	...
	//Kevin
        alerts.addAlert('<strong>Oh Snap!</strong> Could not connect to the aria2 RPC server. Will retry in 5 secs. You might want to check the connection settings by going to Settings > Connection Settings', 'error');
    ...
    syscall.invoke({
      ...
      success: function(data) {
        ...
        if (failed) {
		//Kevin
          alerts.addAlert('<strong>Oh Snap!</strong> Authentication failed while connecting to Aria2 RPC server. Will retry in 5 secs. You might want to confirm your authentication details  by going to Settings > Connection Settings', 'error');
          timeout = setTimeout(update, 5000);
          return;
        }
